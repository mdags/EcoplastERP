...
{
    public partial class Eco2MachineLoadForm : XtraForm, IXpoSessionAwareControl
    {
        public XafApplication winApplication;

        public Eco2MachineLoadForm()
        {
            InitializeComponent();
        }
        ...
        private void Eco2MachineLoadForm_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter(@"select M.Code from Machine M inner join Station S on S.Oid = M.Station where M.GCRecord is null and S.Code = 'Eco2' order by M.Code", ((XPObjectSpace)objectSpace).Session.ConnectionString);
            da.Fill(dt);
        ...
        void item_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                ...
                FilmingWorkOrder workOrder = objectSpace.FindObject<FilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", row["Üretim Siparişi No"].ToString()));
                if (workOrder != null)
                {
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                ((XPObjectSpace)objectSpace).Session.ExecuteNonQuery(string.Format("update FilmingWorkOrder set SequenceNumber = {0} where WorkOrderNumber = '{1}'", e.Value, gridView1.GetRowCellValue(e.RowHandle, "Üretim Siparişi No")));
            }
        }
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            FilmingWorkOrder obj = objectSpace.FindObject<FilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (obj != null)
            {
        ...
        private void btnClone_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            var workOrder = objectSpace.FindObject<FilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (workOrder != null)
            {
                ...
                obj.RollCount = workOrder.RollCount;
                obj.MinimumRollWeight = workOrder.MinimumRollWeight;
                obj.MaximumRollWeight = workOrder.MaximumRollWeight;
                obj.WayCount = workOrder.WayCount;
                obj.GramM2 = workOrder.GramM2;
                obj.GramMetretul = workOrder.GramMetretul;
                obj.InflationRate = workOrder.InflationRate;
                ...
                obj.PaletteLayout = workOrder.PaletteLayout;
                ...
                obj.Lenght = workOrder.Lenght;
                obj.BellowsStatus = workOrder.BellowsStatus;
                obj.Bellows = workOrder.Bellows;
                obj.Corona = workOrder.Corona;
                obj.CoronaPartial = workOrder.CoronaPartial;
                obj.PrintName = workOrder.PrintName;
                obj.Density = workOrder.Density;
                obj.RollWeight = workOrder.RollWeight;
                obj.ShippingPackageType = workOrder.ShippingPackageType;
                obj.ShippingFilmType = workOrder.ShippingFilmType;
                obj.MaterialColor = workOrder.MaterialColor;
                obj.Embossing = workOrder.Embossing;

                obj.PartString = workOrder.PartString;
                obj.RecieptString = workOrder.RecieptString;

                #region ısıl değerler kopyala
                obj.LineSpeed = workOrder.LineSpeed;
                obj.CoolingLine = workOrder.CoolingLine;
                obj.FilmingTension = workOrder.FilmingTension;
                obj.WrapperTension = workOrder.WrapperTension;
                obj.BubbleCooling = workOrder.BubbleCooling;
                obj.IBCFeed = workOrder.IBCFeed;
                obj.IBCExtruder = workOrder.IBCExtruder;
                obj.TowerSpeed = workOrder.TowerSpeed;
                obj.Neck = workOrder.Neck;
                obj.Head1 = workOrder.Head1;
                obj.Head2 = workOrder.Head2;
                obj.Lip = workOrder.Lip;
                #endregion

                foreach (var item in workOrder.FilmingWorkOrderParts)
                {
                    FilmingWorkOrderPart workOrderPart = objectSpace.CreateObject<FilmingWorkOrderPart>();
                    workOrderPart.FilmingWorkOrder = obj;
                    workOrderPart.MachinePart = objectSpace.FindObject<MachinePart>(new BinaryOperator("Oid", item.MachinePart.Oid));
                    workOrderPart.Thickness = item.Thickness;
                }
                foreach (var item in workOrder.FilmingWorkOrderReciepts)
                {
                    FilmingWorkOrderReciept workOrderReciept = objectSpace.CreateObject<FilmingWorkOrderReciept>();
                    workOrderReciept.FilmingWorkOrder = obj;
                    workOrderReciept.MachinePart = objectSpace.FindObject<MachinePart>(new BinaryOperator("Oid", item.MachinePart.Oid));
                    ...
                    workOrderReciept.WorkOrderRate = item.WorkOrderRate;
                }
                ShowViewParameters svp = new ShowViewParameters()
                {
                    CreatedView = winApplication.CreateDetailView(objectSpace, obj),
        ...
        private void btnRezervation_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                var workOrder = objectSpace.FindObject<FilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetRowCellValue(i, "Üretim Siparişi No")));
        ...
        private void btnPrint_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            ReportData reportData = objectSpace.FindObject<ReportData>(new BinaryOperator("Name", "Eco2 Üretim Siparişi Formu"));
            if (reportData != null)
            {
