...
{
    public partial class Eco5RegeneratedMachineLoadForm : XtraForm, IXpoSessionAwareControl
    {
        public XafApplication winApplication;

        public Eco5RegeneratedMachineLoadForm()
        {
            InitializeComponent();
        }
        ...
        private void Eco2MachineLoadForm_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter(@"select M.Code from Machine M inner join Station S on S.Oid = M.Station where M.GCRecord is null and S.Code = 'Eco5 Rejenere' order by M.Code", ((XPObjectSpace)objectSpace).Session.ConnectionString);
            da.Fill(dt);
        ...
        void RefreshGrid(string machineCode)
        {
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            DataTable dt = new DataTable();
            string sqlText = string.Format(@"select (case when W.WorkOrderStatus = 100 then 'Üretim Bekliyor' when W.WorkOrderStatus = 101 then 'Aktif' when W.WorkOrderStatus = 102 then 'Üretim Durduruldu' when W.WorkOrderStatus = 110 then 'Üretim Tamamlandı' when W.WorkOrderStatus = 900 then 'İptal Edildi' else 'NULL' end) as Durumu, W.SequenceNumber as [Sıra No], C.Name as [Müşteri], M.Code as [Stok Kodu], W.WorkName as [İşin Adı], W.WorkOrderNumber as [Üretim Siparişi No], O.OrderNumber+'/'+cast(D.LineNumber as varchar(5)) as [Sipariş No], O.OrderDate as [Sipariş Tarihi], D.LineDeliveryDate as [Termin Tarihi], D.Quantity as [Sipariş Miktarı], W.Quantity as [Üretim Siparişi Miktarı], isnull(sum(case when P.GCRecord is null then P.GrossQuantity else 0 end), 0) as [Toplam Üretim], isnull(W.Quantity - sum(case when P.GCRecord is null then P.GrossQuantity else 0 end), 0) as [Kalan Üretim], isnull((select SUM(GrossQuantity) from Production where GCRecord is null and SalesOrderDetail = D.Oid and IsLastProduction = 1), 0) as [Sipariş Üretilen], isnull((W.Quantity - sum(case when P.GCRecord is null then P.GrossQuantity else 0 end)) / ((case when A.Capacity = 0 then 1 else A.Capacity end)/24), 0) as [Kalan Süre (saat)], W.WorkOrderDate as [Üretim Siparişi Tarihi] from CastRegeneratedWorkOrder W inner join SalesOrderDetail D on D.Oid = W.SalesOrderDetail inner join SalesOrder O on O.Oid = D.SalesOrder inner join Contact C on C.Oid = O.Contact inner join Product M on M.Oid = D.Product left outer join Production P on P.WorkOrderNumber = W.WorkOrderNumber inner join Machine A on A.Oid = W.Machine where W.GCRecord is null And W.WorkOrderStatus != 110 And W.WorkOrderStatus != 900 And A.Code = '{0}'group by W.Oid, W.WorkOrderStatus, W.SequenceNumber, W.WorkOrderNumber, W.WorkOrderDate, W.WorkName,C.Name, D.Oid, O.OrderNumber, O.OrderDate, D.LineDeliveryDate, M.Code, D.Quantity, W.Quantity, A.Capacity, D.LineNumber order by W.SequenceNumber", machineCode);
        ...
        void item_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                ...
                CastRegeneratedWorkOrder workOrder = objectSpace.FindObject<CastRegeneratedWorkOrder>(new BinaryOperator("WorkOrderNumber", row["Üretim Siparişi No"].ToString()));
                if (workOrder != null)
                {
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                ((XPObjectSpace)objectSpace).Session.ExecuteNonQuery(string.Format("update CastRegeneratedWorkOrder set SequenceNumber = {0} where WorkOrderNumber = '{1}'", e.Value, gridView1.GetRowCellValue(e.RowHandle, "Üretim Siparişi No")));
            }
        }
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            CastRegeneratedWorkOrder obj = objectSpace.FindObject<CastRegeneratedWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (obj != null)
            {
        ...
        private void btnClone_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            var workOrder = objectSpace.FindObject<CastRegeneratedWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (workOrder != null)
            {
                ...
                obj.MaxiumPressure = workOrder.MaxiumPressure;
                obj.ExtruderCurent = workOrder.ExtruderCurent;
                obj.AkromelCurrent = workOrder.AkromelCurrent;
                obj.AkromelSpeed = workOrder.AkromelSpeed;

                obj.RecieptString = workOrder.RecieptString;
                foreach (var item in workOrder.CastRegeneratedWorkOrderReciepts)
                {
                    if (item.Product != workOrder.SalesOrderDetail.Product)
                    {
                        CastRegeneratedWorkOrderReciept workOrderReciept = objectSpace.CreateObject<CastRegeneratedWorkOrderReciept>();
                        workOrderReciept.CastRegeneratedWorkOrder = obj;
        ...
        private void btnRezervation_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                var workOrder = objectSpace.FindObject<CastRegeneratedWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetRowCellValue(i, "Üretim Siparişi No")));
        ...
        private void btnPrint_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            ReportData reportData = objectSpace.FindObject<ReportData>(new BinaryOperator("Name", "Eco3 Rejenere Üretim Siparişi Formu"));
            if (reportData != null)
            {
