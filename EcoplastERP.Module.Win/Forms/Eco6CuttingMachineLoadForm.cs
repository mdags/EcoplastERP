...
{
    public partial class Eco6CuttingMachineLoadForm : XtraForm, IXpoSessionAwareControl
    {
        public XafApplication winApplication;

        public Eco6CuttingMachineLoadForm()
        {
            InitializeComponent();
        }
        ...
        void item_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                ...
                Eco6CuttingWorkOrder workOrder = objectSpace.FindObject<Eco6CuttingWorkOrder>(new BinaryOperator("WorkOrderNumber", row["Üretim Siparişi No"].ToString()));
                if (workOrder != null)
                {
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                ((XPObjectSpace)objectSpace).Session.ExecuteNonQuery(string.Format("update Eco6CuttingWorkOrder set SequenceNumber = {0} where WorkOrderNumber = '{1}'", e.Value, gridView1.GetRowCellValue(e.RowHandle, "Üretim Siparişi No")));
            }
        }
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            Eco6CuttingWorkOrder obj = objectSpace.FindObject<Eco6CuttingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (obj != null)
            {
        ...
        private void btnClone_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            var workOrder = objectSpace.FindObject<Eco6CuttingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (workOrder != null)
            {
                ...
                foreach (var item in workOrder.Eco6CuttingWorkOrderReciepts)
                {
                    if (item.Product != workOrder.SalesOrderDetail.Product)
                    {
                        Eco6CuttingWorkOrderReciept workOrderReciept = objectSpace.CreateObject<Eco6CuttingWorkOrderReciept>();
                        workOrderReciept.Eco6CuttingWorkOrder = obj;
        ...
        private void btnRezervation_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                var workOrder = objectSpace.FindObject<Eco6CuttingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetRowCellValue(i, "Üretim Siparişi No")));
        ...
        private void btnPrint_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            ReportData reportData = objectSpace.FindObject<ReportData>(new BinaryOperator("Name", "Eco6 Kesim Üretim Siparişi Formu"));
            if (reportData != null)
            {
