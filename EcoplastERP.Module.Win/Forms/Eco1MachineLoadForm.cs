...
{
    public partial class Eco1MachineLoadForm : XtraForm, IXpoSessionAwareControl
    {
        public XafApplication winApplication;

        public Eco1MachineLoadForm()
        {
            InitializeComponent();
        }
        ...
        private void Eco2MachineLoadForm_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter(@"select M.Code from Machine M inner join Station S on S.Oid = M.Station where M.GCRecord is null and S.Code = 'Eco1' order by M.Code", ((XPObjectSpace)objectSpace).Session.ConnectionString);
            da.Fill(dt);
        ...
        void item_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                ...
                PrintingWorkOrder workOrder = objectSpace.FindObject<PrintingWorkOrder>(new BinaryOperator("WorkOrderNumber", row["Üretim Siparişi No"].ToString()));
                if (workOrder != null)
                {
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                ((XPObjectSpace)objectSpace).Session.ExecuteNonQuery(string.Format("update PrintingWorkOrder set SequenceNumber = {0} where WorkOrderNumber = '{1}'", e.Value, gridView1.GetRowCellValue(e.RowHandle, "Üretim Siparişi No")));
            }
        }
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            PrintingWorkOrder obj = objectSpace.FindObject<PrintingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (obj != null)
            {
        ...
        private void btnClone_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            var workOrder = objectSpace.FindObject<PrintingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (workOrder != null)
            {
                ...
                obj.RollDirection = workOrder.RollDirection;
                obj.RollCount = workOrder.RollCount;
                obj.BandCount = workOrder.BandCount;
                obj.Anilox = workOrder.Anilox;
                obj.ColorCount = workOrder.ColorCount;
                obj.PrintingColors = workOrder.PrintingColors;
                obj.PrintForm = workOrder.PrintForm;
                obj.PrintDirection = workOrder.PrintDirection;
                obj.PrintWorkStatus = workOrder.PrintWorkStatus;
                obj.ClicheBand = workOrder.ClicheBand;
                obj.ClicheThickness = workOrder.ClicheThickness;
                obj.ClicheCode = workOrder.ClicheCode;
                obj.CylinderRepetition = workOrder.CylinderRepetition;
                obj.Logo = workOrder.Logo;
                obj.Barcode = workOrder.Barcode;
                ...
                foreach (var item in workOrder.PrintingWorkOrderReciepts)
                {
                    if (item.Product != workOrder.SalesOrderDetail.Product)
                    {
                        PrintingWorkOrderReciept workOrderReciept = objectSpace.CreateObject<PrintingWorkOrderReciept>();
                        workOrderReciept.PrintingWorkOrder = obj;
        ...
        private void btnRezervation_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                var workOrder = objectSpace.FindObject<PrintingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetRowCellValue(i, "Üretim Siparişi No")));
        ...
        private void btnPrint_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            ReportData reportData = objectSpace.FindObject<ReportData>(new BinaryOperator("Name", "Eco1 Üretim Siparişi Formu"));
            if (reportData != null)
            {
