...
{
    public partial class Eco5StretchMachineLoadForm : XtraForm, IXpoSessionAwareControl
    {
        public XafApplication winApplication;

        public Eco5StretchMachineLoadForm()
        {
            InitializeComponent();
        }
        ...
        private void Eco2MachineLoadForm_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter(@"select M.Code from Machine M inner join Station S on S.Oid = M.Station where M.GCRecord is null and S.Code = 'Eco5 Stretch' order by M.Code", ((XPObjectSpace)objectSpace).Session.ConnectionString);
            da.Fill(dt);
        ...
        void item_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                ...
                CastFilmingWorkOrder workOrder = objectSpace.FindObject<CastFilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", row["Üretim Siparişi No"].ToString()));
                if (workOrder != null)
                {
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                ((XPObjectSpace)objectSpace).Session.ExecuteNonQuery(string.Format("update CastFilmingWorkOrder set SequenceNumber = {0} where WorkOrderNumber = '{1}'", e.Value, gridView1.GetRowCellValue(e.RowHandle, "Üretim Siparişi No")));
            }
        }
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            CastFilmingWorkOrder obj = objectSpace.FindObject<CastFilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (obj != null)
            {
        ...
        private void btnClone_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            var workOrder = objectSpace.FindObject<CastFilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (workOrder != null)
            {
                ...
                obj.CapStatus = workOrder.CapStatus;
                obj.Cap = workOrder.Cap;
                ...
                if (workOrder.OuterPacking != null) obj.OuterPacking = objectSpace.FindObject<OuterPacking>(new BinaryOperator("Oid", workOrder.OuterPacking.Oid));
                ...
                foreach (var item in workOrder.CastFilmingWorkOrderParts)
                {
                    CastFilmingWorkOrderPart workOrderPart = objectSpace.CreateObject<CastFilmingWorkOrderPart>();
                    workOrderPart.CastFilmingWorkOrder = obj;
                ...
                foreach (var item in workOrder.CastFilmingWorkOrderReciepts)
                {
                    CastFilmingWorkOrderReciept workOrderReciept = objectSpace.CreateObject<CastFilmingWorkOrderReciept>();
                    workOrderReciept.CastFilmingWorkOrder = obj;
        ...
        private void btnRezervation_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                var workOrder = objectSpace.FindObject<CastFilmingWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetRowCellValue(i, "Üretim Siparişi No")));
        ...
        private void btnPrint_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            ReportData reportData = objectSpace.FindObject<ReportData>(new BinaryOperator("Name", "Eco5 Stretch Üretim Siparişi Formu"));
            if (reportData != null)
            {
