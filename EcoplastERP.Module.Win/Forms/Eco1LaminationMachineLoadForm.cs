...
{
    public partial class Eco1LaminationMachineLoadForm : XtraForm, IXpoSessionAwareControl
    {
        public XafApplication winApplication;

        public Eco1LaminationMachineLoadForm()
        {
            InitializeComponent();
        }
        ...
        private void Eco2MachineLoadForm_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter(@"select M.Code from Machine M inner join Station S on S.Oid = M.Station where M.GCRecord is null and S.Code = 'Eco1 Laminasyon' order by M.Code", ((XPObjectSpace)objectSpace).Session.ConnectionString);
            da.Fill(dt);
        ...
        void item_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                ...
                LaminationWorkOrder workOrder = objectSpace.FindObject<LaminationWorkOrder>(new BinaryOperator("WorkOrderNumber", row["Üretim Siparişi No"].ToString()));
                if (workOrder != null)
                {
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                ((XPObjectSpace)objectSpace).Session.ExecuteNonQuery(string.Format("update LaminationWorkOrder set SequenceNumber = {0} where WorkOrderNumber = '{1}'", e.Value, gridView1.GetRowCellValue(e.RowHandle, "Üretim Siparişi No")));
            }
        }
        ...
        private void gridView1_DoubleClick(object sender, EventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            LaminationWorkOrder obj = objectSpace.FindObject<LaminationWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (obj != null)
            {
        ...
        private void btnClone_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            var workOrder = objectSpace.FindObject<LaminationWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetFocusedRowCellValue("Üretim Siparişi No")));
            if (workOrder != null)
            {
                ...
                foreach (var item in workOrder.LaminationWorkOrderReciepts)
                {
                    if (item.Product != workOrder.SalesOrderDetail.Product)
                    {
                        LaminationWorkOrderReciept workOrderReciept = objectSpace.CreateObject<LaminationWorkOrderReciept>();
                        workOrderReciept.LaminationWorkOrder = obj;
        ...
        private void btnRezervation_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                var workOrder = objectSpace.FindObject<LaminationWorkOrder>(new BinaryOperator("WorkOrderNumber", gridView1.GetRowCellValue(i, "Üretim Siparişi No")));
        ...
        private void btnPrint_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (gridView1.FocusedValue == null) return;
            IObjectSpace objectSpace = winApplication.CreateObjectSpace();
            ReportData reportData = objectSpace.FindObject<ReportData>(new BinaryOperator("Name", "Eco1 Laminasyon Üretim Siparişi Formu"));
            if (reportData != null)
            {
