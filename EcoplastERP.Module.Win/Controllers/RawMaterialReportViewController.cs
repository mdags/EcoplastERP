...
{
    public partial class RawMaterialReportViewController : ViewController
    {
        public RawMaterialReportViewController()
        {
            InitializeComponent();
        ...
        private void RefreshAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            ...
            if (cvi != null)
            {
                RawMaterialReportUserControl userControl = cvi.Control as RawMaterialReportUserControl;
        ...
        private void RawMaterialReportSelectTypeAction_Execute(object sender, SingleChoiceActionExecuteEventArgs e)
        {
            ControlViewItem cvi = ((DashboardView)View).Items[0] as ControlViewItem;
            if (cvi != null)
            {
                RawMaterialReportUserControl userControl = cvi.Control as RawMaterialReportUserControl;
                if (e.SelectedChoiceActionItem.Id == "Reçete Bazında") userControl.reportID = 1;
                if (e.SelectedChoiceActionItem.Id == "Çıkış Hareketi Bazında") userControl.reportID = 2;
            }
        }

        private void ExportRawMaterialReportAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            ControlViewItem cvi = ((DashboardView)View).Items[0] as ControlViewItem;
            if (cvi != null)
            {
