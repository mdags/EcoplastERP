...
/****** Object:  StoredProcedure [dbo].[SP_ShippingByStandartPalette]    Script Date: 12.01.2017 10:54:34 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[SP_ShippingByStandartPalette] 
	@PLANCODE		varchar(100),
	@PALETTEID		varchar(100),
	@ORCODE			nvarchar(100),
	@IsCancel		bit,
	@Status			int,
	@Message		varchar(100)
...
BEGIN
	...
	BEGIN
		...
		IF((SELECT TOP 1 MATERIALID from STORES where PALETTENO = @PALETTEID) != (SELECT TOP 1 MATERIALID FROM ORDERLINES where ORCODE = @ORCODE AND ORDERLINENO = 0))
		...
		BEGIN
			...
			UPDATE STORES SET ORCODE = @ORCODE, STOREID = 135, EDATE = GETDATE() WHERE PALETTENO = @PALETTEID AND STOREID = 121 
		END
	END
	ELSE
	BEGIN
		...
		SELECT @OLDORCODE = ORCODE FROM PRODUCTS WHERE PRODUCTID = (SELECT TOP 1 PRODUCTID FROM SHIPPINGS WHERE PLANCODE = @PLANCODE AND PALETTEID = @PALETTEID AND ORCODE = @ORCODE)
        delete SHIPPINGS where STORESID in (SELECT STORESID FROM STORES WHERE PALETTENO = @PALETTEID AND STOREID = 135)
		UPDATE STORES SET ORCODE = @OLDORCODE, STOREID = 121, EDATE = GETDATE() WHERE PALETTENO = @PALETTEID AND STOREID = 135 
	END
