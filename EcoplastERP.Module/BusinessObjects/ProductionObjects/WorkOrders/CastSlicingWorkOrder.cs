...
namespace EcoplastERP.Module.BusinessObjects.ProductionObjects
{
    ...
    [Appearance("CastSlicingWorkOrder.LineDeliveryDateAppearance", TargetItems = "WorkName", Criteria = "SalesOrderDetail.LineDeliveryDate < LocalDateTimeToday()", BackColor = "MistyRose")]
    [Appearance("CastSlicingWorkOrder.TotalProductionQuantityAppearance", TargetItems = "TotalProduction", Criteria = "TotalProduction > Quantity", BackColor = "GreenYellow")]
    [Appearance("CastSlicingWorkOrder.TotalProductionQuantityAppearance1", TargetItems = "TotalProduction", Criteria = "TotalProduction <= Quantity", BackColor = "LemonChiffon")]
    [Appearance("CastSlicingWorkOrder.CompletedAppearance", TargetItems = "*", Criteria = "WorkOrderStatus = 'ProductionComplete'", FontStyle = FontStyle.Strikeout, BackColor = "LemonChiffon")]
    [Appearance("CastSlicingWorkOrder.CanceledAppearance", TargetItems = "*", Criteria = "WorkOrderStatus = 'Canceled'", FontStyle = FontStyle.Strikeout, BackColor = "MistyRose")]
    [Appearance("CastSlicingWorkOrder.QuarantineQuantityAppearance", TargetItems = "*", Criteria = "QuarantineQuantity > 0", BackColor = "PowderBlue", Context = "CastSlicingWorkOrder_ListView")]
    [Appearance("CastSlicingWorkOrder_MachineLoad.QuarantineQuantityAppearance", TargetItems = "*", Criteria = "QuarantineQuantity > 0", BackColor = "PowderBlue", Context = "CastSlicingWorkOrder_ListView_MachineLoad")]
    public class CastSlicingWorkOrder : BaseObject
    {
        public CastSlicingWorkOrder(Session session)
            : base(session)
        {
        }
        public override void AfterConstruction()
        {
            ...
            Station = Session.FindObject<Station>(new BinaryOperator("Name", "Cast Dilme"));
        ...
        protected override void OnSaving()
        {
            ...
            if (WorkOrderStatus == WorkOrderStatus.ProductionComplete || WorkOrderStatus == WorkOrderStatus.Canceled)
                SequenceNumber = 9000;
            ...
            //CastSlicingWorkOrderReciepts.Sorting.Add(new SortProperty("Product.Name", DevExpress.Xpo.DB.SortingDirection.Ascending));
            //var sbReciept = new StringBuilder();
            //foreach (var reciept in CastSlicingWorkOrderReciepts)
            //{
        ...
        [Appearance("CastSlicingWorkOrder.WorkName", Context = "DetailView", Enabled = false, Criteria = "1 = 1")]
        ...
        public decimal TotalProduction
        {
            get
            {
                return Convert.ToDecimal(Session.Evaluate(typeof(Production), CriteriaOperator.Parse("Sum(GrossQuantity)"), CriteriaOperator.Parse("CastSlicingWorkOrder = ?", Oid)));
            }
        }
        ...
        public decimal TotalWastage
        {
            get
            {
                return Convert.ToDecimal(Session.Evaluate(typeof(Wastage), CriteriaOperator.Parse("Sum(GrossQuantity)"), CriteriaOperator.Parse("CastSlicingWorkOrder = ?", Oid)));
            }
        }
        ...
        [Appearance("CastSlicingWorkOrder.PaletteStickerDesign", Context = "DetailView", Visibility = ViewItemVisibility.Hide, Criteria = "NextStation.Name != 'Son Üretim'")]
        ...
        [Appearance("CastSlicingWorkOrder.PaletteNumber", Context = "DetailView", Enabled = false, Criteria = "1 = 1")]
        [VisibleInDetailView(false)]
        ...
        public XPCollection<CastSlicingWorkOrderReciept> CastSlicingWorkOrderReciepts
        {
            get { return GetCollection<CastSlicingWorkOrderReciept>("CastSlicingWorkOrderReciepts"); }
        }
        ...
        void GetSalesOrderDetail()
        {
            ...
            {
                var castSlicingWorkOrder = Session.FindObject<CastSlicingWorkOrder>(new BinaryOperator("SalesOrderDetail", SalesOrderDetail.Oid));
                if (castSlicingWorkOrder != null) XtraMessageBox.Show(string.Format("Bu müşteri siparişi için daha önce {0} numaralı üretim siparişi doldurulmuş. Mükerrer bir kayıt olup olmadığını kontrol ediniz...", castSlicingWorkOrder.WorkOrderNumber), "Bilgilendirme", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information);
                ...
                Session.Delete(CastSlicingWorkOrderReciepts);
                CastSlicingWorkOrderReciept reciept = new CastSlicingWorkOrderReciept(Session)
                {
                    CastSlicingWorkOrder = this,
                ...
                CastSlicingWorkOrderReciepts.Add(reciept);
            }
        }
        ...
        void GetReciept()
        {
            ...
            if (Reciept != null)
            {
                ...
                {
                    var slicingWorkOrderReciept = new CastSlicingWorkOrderReciept(Session)
                    {
                        CastSlicingWorkOrder = this,
                    ...
                    this.CastSlicingWorkOrderReciepts.Add(slicingWorkOrderReciept);
                }
            ...
            if (SalesOrderDetail != null)
            {
                if (SalesOrderDetail.Product != null)
                {
                    if (SalesOrderDetail.Product.ProductKind != null)
                    {
                        ...
                        {
                            int width = 0;
                            if (SalesOrderDetail.Product.BellowsStatus == BellowsStatus.None) width = Width;
                            else
                            {
                                decimal bellow = Convert.ToDecimal(SalesOrderDetail.Product.Bellows);
                                width = SalesOrderDetail.Product.ShippingFilmType.Name.Contains("HORTUM") ? Width + Convert.ToInt32(bellow * 4) : Width + Convert.ToInt32(bellow * 2);
                            }
                            decimal hangingPlaceValue = SalesOrderDetail.Product.HangingLocation == HangingLocation.Does ? 0.01M : 0;
                            int thicknessPart = SalesOrderDetail.Product.ShippingFilmType.ThicknessDoublePart ? 2 : 1;

                            if (SalesOrderDetail.Product.ProductKind.Name.Contains("TORBA"))
                            {
                                if (SalesOrderDetail.Product.ShippingFilmType.Name == "YAPRAK")
                                {
                                    int height = 1000;
                                    decimal en = height + height + width + width + SalesOrderDetail.Product.Cap + SalesOrderDetail.Product.Cap;
                                    GramMetretul = ((en * height * Density * Thickness) / 100000) + hangingPlaceValue;
                                }
                                else if (SalesOrderDetail.Product.ShippingFilmType.Name.Contains("HORTUM"))
                                {
                                    int height = 1000;
