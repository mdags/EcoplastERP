...
{
    public partial class Eco6CuttingWorkOrderViewController : ViewController
    {
        private ChoiceActionItem setStatusItem;

        public Eco6CuttingWorkOrderViewController()
        {
            InitializeComponent();

            SetEco6CuttingWorkOrderStatusAction.Items.Clear();
            setStatusItem = new ChoiceActionItem(CaptionHelper.GetMemberCaption(typeof(Eco6CuttingWorkOrder), "WorkOrderStatus"), null);
            SetEco6CuttingWorkOrderStatusAction.Items.Add(setStatusItem);
        ...
        protected override void OnActivated()
        {
            ...
            this.SetEco6CuttingWorkOrderStatusAction.Active.SetItemValue("ObjectType", Helpers.IsUserAdministrator() || Helpers.IsUserInRole("Eco6 Planlama") ? true : false);
            this.PrintEco6CuttingMachineLoadAction.Active.SetItemValue("ObjectType", Helpers.IsUserAdministrator() || Helpers.IsUserInRole("Eco6 Planlama") ? true : false);
        }
        ...
        private void SetEco6CuttingWorkOrderStatusAction_Execute(object sender, SingleChoiceActionExecuteEventArgs e)
        {
            ...
            if (e.SelectedChoiceActionItem.ParentItem == setStatusItem)
            {
                ...
                {
                    Eco6CuttingWorkOrder objInNewObjectSpace = (Eco6CuttingWorkOrder)objectSpace.GetObject(obj);
        ...
        private void ViewEco6CuttingWorkOrderAction_Execute(object sender, SingleChoiceActionExecuteEventArgs e)
        {
            IObjectSpace objectSpace = Application.CreateObjectSpace();
            Eco6CuttingWorkOrder workOrder = (Eco6CuttingWorkOrder)objectSpace.GetObject(e.CurrentObject);
            const string workOrder_ListView = "Eco6CuttingWorkOrder_ListView_Copy";

            if (e.SelectedChoiceActionItem.Caption == "Aynı Firmanın Aynı Stoğu")
            {
                if (workOrder == null) return;
                CollectionSourceBase collectionSource = Application.CreateCollectionSource(objectSpace, typeof(Eco6CuttingWorkOrder), workOrder_ListView);
        ...
        private void PrintEco6CuttingMachineLoadAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            IObjectSpace objectSpace = Application.CreateObjectSpace();
            IReportDataV2 reportData = objectSpace.FindObject<ReportDataV2>(new BinaryOperator("DisplayName", "Eco6 Kesim Makine Yükü"));
            if (reportData != null)
            {
