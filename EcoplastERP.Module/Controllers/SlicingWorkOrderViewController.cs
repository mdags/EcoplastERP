...
{
    public partial class SlicingWorkOrderViewController : ViewController
    {
        private ChoiceActionItem setStatusItem;

        public SlicingWorkOrderViewController()
        {
            InitializeComponent();

            SetSlicingWorkOrderStatusAction.Items.Clear();
            setStatusItem = new ChoiceActionItem(CaptionHelper.GetMemberCaption(typeof(SlicingWorkOrder), "WorkOrderStatus"), null);
            SetSlicingWorkOrderStatusAction.Items.Add(setStatusItem);
        ...
        protected override void OnActivated()
        {
            ...
            this.SetSlicingWorkOrderStatusAction.Active.SetItemValue("ObjectType", Helpers.IsUserAdministrator() || Helpers.IsUserInRole("Eco4 Planlama") ? true : false);
            this.PrintSlicingMachineLoadAction.Active.SetItemValue("ObjectType", Helpers.IsUserAdministrator() || Helpers.IsUserInRole("Eco4 Planlama") ? true : false);
        }
        ...
        private void SetSlicingWorkOrderStatusAction_Execute(object sender, SingleChoiceActionExecuteEventArgs e)
        {
            ...
            if (e.SelectedChoiceActionItem.ParentItem == setStatusItem)
            {
                ...
                {
                    SlicingWorkOrder objInNewObjectSpace = (SlicingWorkOrder)objectSpace.GetObject(obj);
        ...
        private void ViewSlicingWorkOrderAction_Execute(object sender, SingleChoiceActionExecuteEventArgs e)
        {
            IObjectSpace objectSpace = Application.CreateObjectSpace();
            SlicingWorkOrder workOrder = (SlicingWorkOrder)objectSpace.GetObject(e.CurrentObject);
            const string workOrder_ListView = "SlicingWorkOrder_ListView_Copy";

            if (e.SelectedChoiceActionItem.Caption == "Aynı Firmanın Aynı Stoğu")
            {
                if (workOrder == null) return;
                CollectionSourceBase collectionSource = Application.CreateCollectionSource(objectSpace, typeof(SlicingWorkOrder), workOrder_ListView);
        ...
        private void PrintSlicingMachineLoadAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            IObjectSpace objectSpace = Application.CreateObjectSpace();
            IReportDataV2 reportData = objectSpace.FindObject<ReportDataV2>(new BinaryOperator("DisplayName", "Eco4 Dilme Makine Yükü"));
            if (reportData != null)
            {
