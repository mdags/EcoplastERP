...
{
    public partial class Eco6LaminationWorkOrderViewController : ViewController
    {
        private ChoiceActionItem setStatusItem;

        public Eco6LaminationWorkOrderViewController()
        {
            InitializeComponent();

            SetEco6LaminationWorkOrderStatusAction.Items.Clear();
            setStatusItem = new ChoiceActionItem(CaptionHelper.GetMemberCaption(typeof(LaminationWorkOrder), "WorkOrderStatus"), null);
            SetEco6LaminationWorkOrderStatusAction.Items.Add(setStatusItem);
        ...
        protected override void OnActivated()
        {
            ...
            this.SetEco6LaminationWorkOrderStatusAction.Active.SetItemValue("ObjectType", Helpers.IsUserAdministrator() || Helpers.IsUserInRole("Eco6 Planlama") ? true : false);
            this.PrintEco6LaminationMachineLoadAction.Active.SetItemValue("ObjectType", Helpers.IsUserAdministrator() || Helpers.IsUserInRole("Eco6 Planlama") ? true : false);
        }
        ...
        private void SetLaminationWorkOrderStatusAction_Execute(object sender, SingleChoiceActionExecuteEventArgs e)
        {
            ...
            if (e.SelectedChoiceActionItem.ParentItem == setStatusItem)
            {
                ...
                {
                    Eco6LaminationWorkOrder objInNewObjectSpace = (Eco6LaminationWorkOrder)objectSpace.GetObject(obj);
        ...
        private void ViewEco6LaminationWorkOrderAction_Execute(object sender, SingleChoiceActionExecuteEventArgs e)
        {
            IObjectSpace objectSpace = Application.CreateObjectSpace();
            Eco6LaminationWorkOrder workOrder = (Eco6LaminationWorkOrder)objectSpace.GetObject(e.CurrentObject);
            const string workOrder_ListView = "Eco6LaminationWorkOrder_ListView_Copy";

            if (e.SelectedChoiceActionItem.Caption == "Aynı Firmanın Aynı Stoğu")
            {
                if (workOrder == null) return;
                CollectionSourceBase collectionSource = Application.CreateCollectionSource(objectSpace, typeof(Eco6LaminationWorkOrder), workOrder_ListView);
        ...
        private void PrintEco6LaminationMachineLoadAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            IObjectSpace objectSpace = Application.CreateObjectSpace();
            IReportDataV2 reportData = objectSpace.FindObject<ReportDataV2>(new BinaryOperator("DisplayName", "Eco6 Laminasyon Makine Yükü"));
            if (reportData != null)
            {
